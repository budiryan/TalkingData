...
'''
# Limit the training of RF to save memory, but for LGB train on entire dataset
'''
...
if __name__ == '__main__':
    ...
    num_train_random_forest = 75000000 + VAL_SIZE # Limit the training of RF to save memory

    print('Training sklearn Random Forest')

    if DEBUG:
        print('Train RF from: ', 0, ' to ', len_train - VAL_SIZE)
        clf.fit(train_df[: len_train - VAL_SIZE][predictors].values, train_df[: len_train][target].values)
    else:
        # do not include the validation dataset
        print('Train RF from: ', len_train - num_train_random_forest, ' to ', len_train - VAL_SIZE)
        clf.fit(train_df[len_train - num_train_random_forest: len_train - VAL_SIZE][predictors].values, 
            train_df[len_train - num_train_random_forest: len_train - VAL_SIZE][target].values)

    print('Predict using the trained random forest')
    train_df.loc[len_train:, target] = clf.predict(train_df[len_train:][predictors].values)
