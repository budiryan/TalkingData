...
WHERE = 'gcloud'
NCHUNK = 100000000
OFFSET = 100000000 # Try no offset
...
def get_df():
    ...
    try:
        ...
        train_df = do_var( train_df, ['ip', 'app', 'channel'], 'hour', 'ip_app_channel_var_hour', show_max=True ); gc.collect() # new
        train_df = do_mean( train_df, ['app', 'channel'], 'hour', 'app_channel_mean_hour', show_max=True ); gc.collect() # new
        train_df = do_var( train_df, ['app', 'channel'], 'hour', 'app_channel_var_hour', show_max=True ); gc.collect() # new
        ...
        train_df[new_feature] = pd.Series(QQ).astype('int16')
        del QQ, next_clicks
        gc.collect()
...
'''
print("Slicing train_df")
train_df = train_df[len(train_df) - 75000000: len(train_df)]
gc.collect()
print("len train df now: ", len(train_df))
'''
...
train_pool = catboost.Pool(train_df[predictors], train_df[target], cat_indices, thread_count=16)
cols = train_df.columns
del train_df
gc.collect()

val_pool = catboost.Pool(val_df[predictors], val_df[target], cat_indices, thread_count=16)
del val_df
gc.collect()
'''

print("Modelling part: CatBoost Classifier")
...
cb.fit(X=train_df[predictors],
       y=train_df[target], 
       eval_set=(val_df[predictors], val_df[target]),
       use_best_model=True, 
       verbose=True)
...
print("Done training, delete train and val data")
del train_df, val_df
gc.collect()
...
y_pred = cb.predict(test_df[predictors])
