...
'''
Impute NaN values using mean instead of zeros,
Use ALL data, tune lgb hyperparameter to be less prone to overfitting
'''
...
if __name__ == '__main__':
    ...
    print('Extract day and hour only from the dataset')
    ...
    train_df['next_click'] = (train_df.groupby(['ip', 'app', 'device', 'os']).click_time.shift(-1)
                              .fillna(3000000000) - train_df.click_time).astype(np.float32)
    train_df['prev_click'] = (train_df.click_time - train_df.groupby(['ip', 'app', 'device', 'os'])
                              .click_time.shift(1).fillna(3000000000)).astype(np.float32)
    train_df.drop(['click_time'], axis=1, inplace=True)
    predictors.extend(['next_click', 'prev_click'])
    gc.collect()
    print('Finished generating next_click and prev_click')

    print('Generating mean encoding using random forrest boosted light gbm')
    ...
    # save before begin testing
    print('save train_df to pickle to save memory')
    train_df.to_pickle(PICKLEPATH + 'all' + VERSION_NUM + '.pkl.gz')

    lgb_params_encoding = {
        'boosting_type': 'rf', # Train using rf instead of boosted decision tree
        'objective': objective,
        'metric': metrics,
        'learning_rate': 0.1,
        ...
        'scale_pos_weight': 500,  # because training data is extremely unbalanced
    ...
    xgtrain_encoding = lgb.Dataset(train_df[: (len_train - VAL_SIZE)][predictors].values,
                                   label=train_df[: (len_train - VAL_SIZE)][target].values,
                          feature_name=predictors,
                          categorical_feature=categorical
                          )
    gc.collect()

    xgvalid_encoding = lgb.Dataset(train_df[(len_train - VAL_SIZE): len_train][predictors].values,
                                   label=train_df[(len_train - VAL_SIZE): len_train][target].values,
                          feature_name=predictors,
                          categorical_feature=categorical
                          )

    print('deleted train_df')
    del train_df
    gc.collect()

    evals_results = {}
    bst_encoding = lgb.train(lgb_params_encoding,
                    xgtrain_encoding,
                    valid_sets=[xgtrain_encoding, xgvalid_encoding],
    ...
    # load here
    print('Re-reading all data')
    train_df = pd.read_pickle(PICKLEPATH + 'all' + VERSION_NUM + '.pkl.gz')

    test_shit = bst_encoding.predict(train_df[len_train:][predictors].values,
                                       num_iteration=bst_encoding.best_iteration)
    test_shit = pd.Series((test_shit > 0.5)).astype('int8')

    train_df.loc[len_train:, target] = test_shit.values

    # print('Num of positive target: ', train_df[len_train:][train_df[len_train: ][target] == 1].shape)
    del xgtrain_encoding, xgvalid_encoding, bst_encoding
    gc.collect()
    ...
    # Impute the expanding mean with imputer instead of zeros
    imputer = Imputer()
    train_df['feat_expanding_mean'] = imputer.fit_transform(train_df['feat_expanding_mean'].values.reshape(-1, 1))
    del cumsum, cumcnt
    gc.collect()
